#!/usr/bin/env python3
"""Generate a random real device identity with valid IMEI"""
...
def generate_identity(db_path, idx_arg="random"):
    ...
    ouis = {'Samsung':['8C:F5:A3','B0:72:BF','AC:5F:3E'],'Xiaomi':['28:6C:07','64:CC:2E'],
            'OPPO':['2C:4D:54','A4:50:46'],'Realme':['2C:4D:54','E8:FB:1C'],
            'Google':['3C:28:6D','F4:F5:D8'],'TECNO':['D8:1E:05','48:A4:72'],
            'POCO':['28:6C:07','DC:D8:7D'],'Motorola':['B4:F1:DA','80:6C:1B'],
            'Vivo':['C8:F8:6D','70:78:8B'],'Oneplus':['94:65:2D','C0:EE:FB'],
            'Huawei':['FC:48:EF','48:DB:50'],'Nokia':['A4:4E:31','8C:90:D3'],
            'Infinix':['D8:1E:05','C4:AD:34']}
    ...
    return {
        'brand': brand, 'model': model_name, 'device': device_code, 'product': device_code,
        'manufacturer': brand, 'fingerprint': fp, 'hardware': hw, 'board': hw,
        'display': f"{build_id}.{build_num}", 'baseband': baseband,
        'imei1': imei1, 'imei2': imei2, 'serial': serial,
        'android_id': android_id, 'gaid': gaid,
        'wifi_mac': wifi_mac, 'bt_mac': bt_mac,
        'density': density, 'android_ver': android_ver, 'specs': specs,
    }

if __name__ == "__main__":
    db = sys.argv[1] if len(sys.argv) > 1 else "spoofdb/tac_database.json"
    idx = sys.argv[2] if len(sys.argv) > 2 else "random"
    out = sys.argv[3] if len(sys.argv) > 3 else None
    
    identity = generate_identity(db, idx)
